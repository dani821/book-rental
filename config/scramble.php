...
use App\Support\JsonApi\Scramble\DomainExceptionResponseExtension;
use App\Support\JsonApi\Scramble\FrameworkExceptionResponseExtension;
...
return [
    ...
    'info' => [
         ...
         */
        'description' => 'JSON:API for the book rental library. Responses use the `application/vnd.api+json` media type with a `data` envelope; errors use a top-level `errors[]` array. Authenticate with a Sanctum bearer token obtained from `register` or `login`.',
    ],
    ...
    /*
     * Render every error response using the application's JSON:API error envelope
     * (see App\Support\JsonApi\JsonApiExceptionRenderer). Registered here so they take
     * precedence over Scramble's built-in (flat) exception responses.
     */
    'extensions' => [
        FrameworkExceptionResponseExtension::class,
        DomainExceptionResponseExtension::class,
    ],
